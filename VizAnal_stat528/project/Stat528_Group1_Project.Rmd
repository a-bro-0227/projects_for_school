...
The first tab was labeled `raw`, while the second was labeled `simple`. To ensure we had all the data possible we did a quick comparison of the two data sets by `left_joining`  the `raw` tab's column names to the `simple` tab's column names and making sure there was no `NA` values. We then ensured the lengths of each tab were the same using `nrow`.
...
This proved that the `simple` tab is just a subset of the `raw` tab. Since the `raw` data is not overly large, we will just use the `raw` tab and filter out columns that really won't help us. This way we will avoid missing any conclusions that can be drawn from the `raw` tab information that cannot be drawn from the `simple` tab. Below is a `glimpse` of the Dallas Animal Shelter columns which seemed potentially meaningful form the `raw` tab.
...
* We added `intake_day` and `outcome_day` which represents the day of the week when the intake or outcome happened, respectively. This will allow us to draw insight to activity on different days of the week.
* We added `total_time` which represents the number or days of the animal was at the shelter.
* We lumped the 20% least common types of dog breeds brought into the Dallas Animal Shelter as `OTHER`. During the analysis of this report, we discovered there were many different types of dog breeds brought into the shelter and many instance where there was only one or two dogs of that particular dog breed type which was brought into the shelter. Lumping the least common dog breeds into one breed type `OTHER` allows for better visual analysis of dog breeds. To do this we used the following steps:
...
In our analysis, we broke in down in three different "events". Intake, Time at Shelter, and Outcome. These sections represent the flow of an animal coming into the shelter, the time at the shelter, and then leaving the shelter.
...
For this section, we will look at the time animals spend at the shelter. An animal shelter obviously houses animals, but sadly, only has room for so many animals. To understand how long animals are in the shelter, we will use the variable we added in the beginning called `total_time` which indicates how many days the animal was in the shelter by using `datediff` on `intake_date` and `outcome_date`. We found that average amount of time an animal spends at the Dallas Animal Shelter is `r round(mean(dallas_animals$total_time, na.rm = T), 2)` days and the time average amount of time that a dog is there before being adopted is is 8.30 days. The table below shows average time at the animal shelter for different `outcome_types`. We can see that if there is an issue with a report, the animal will likely be there a longer time. It is interesting that there are outcome types with reporting issues and makes one wonder what actually physically happens to these animals.
...
To bring the focus more to dogs again, the below `Average Time at Shelter Before Adoption` graph represents the average number of days that a particular dog breed spends at the Dallas Animal Shelter before it is adopted. While this animal shelter's data set does not have "animal size", but from basic knowledge of dog size, we can see that smaller dogs are adopted more quickly.
...
To further investigate this, we created an `adoption_ratio` for the `breed_type` which took the number of that breed that the shelter took in altogether and compared to those that were adopted of that breed. We then filter down to the top ten breed types and plotted the adoption ratio on the x-axis and number the number of days on the y axis to create the `Adoption Ratio by Adoption Days` graph below. We expected to see as the ratio went up, the amount of time it took to adopt the animal to go down. However, that doesn't appear to be the case. The amount of time before a dog gets adopted, again, apears to be more related to the size of dog than the likelyhood that it will be adopted.
...
Next we will do the same type of time series analysis on the `outcome_date` as we did on the `income_date`. The `Outcome Time Series` graph below shows the same seasonality as our intake data, the smoothing line dips in the winter months and then spikes in the spring and summer months. While it is not as drastic of a sign for seasonality, this is not surprising since when it is warm out we want to be outside and walk with dogs, while some people could look at an animal being a burden in the winter time.
...
Moving on, the `Outcome Types` graph below represents the `outcome_types` of animals from the Dallas Animal Shelter ordered from most common `outcome_types` to least common. We can see that `ADOPTION`, `EUTHENIZED`, and `TRANSFER` are the most common. `ADOPTION` and `EUTHENIZED` are expected to be the most common because that is the purpose of an animal shelter.
...
In conclusion, besides wanting to adopt a dog, we believe that the information provided, will be abe to help animal shelters better understand their data in multiple ways to save them time, money, and resources. We found that the Dallas Animal Shelter could have some seasonality in both their intake of dogs and outcome of dogs. We know that the most common intake type for the Dallas Animal Shelter is a stray and that strays are usually found on Wednesday's and most likely, a Pitbull. We found that there may be some dogs which are a little more difficult to deal with than other dogs due to the percent of owner surrenders, but there is no particular dog types which stick out as being extreamly difficult to deal with.

In our analysis when we examined how long animals are at the Dallas Animal Shelter We found that average amount of time an animal spends at the Dallas Animal Shelter is `r round(mean(dallas_animals$total_time, na.rm = T), 2)` days and the time average amount of time that a dog is there before being adopted is is 8.30 days and varys between dog breeds. Overally, we can see that animals are usually adopted withing a week or so, but even shorter amont of time if they are a small dog breed. In addtion to findings about adoption, we found that the Dallas Animal Shelter could possibly have a process or reporting problem which holds dogs up from being adopted.

When looking at the outcome of animals and dogs from the Dallas Animal shelter, we took a closer look at adoption through out the year and tied dates of adoption to holiday's. We found the dallas animal shelter can expect a lot more adoptions right after Thanksgiving and right before Christmas. We know that animals at the Dallas anmial shelter are likely going to be adopted, euthanized, or transfered and that more agressive dog breeds have a higher probability of being euthanized. In addtion, we know what the beginning of the week and Saturdays are usaully busier at the Dallas Animal Shelter.


# Plans for Further work:

To improve this study, we could look at things such as:
...
* When we `glimpsed` at the data you could see there were kennel numbers. If we had enough data we could see if location of animals and adoption rates are related.
* We could break down adoption by month to see if there is busier months than others, or even times to see if the selter is busier at certain times to staff apporpiately.
